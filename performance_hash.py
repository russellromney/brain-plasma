...
s = '--------'
mem,pic,bra,brah = ('in-memory','pickle','brain-plasma','brain-plasma-hash')
brain = Brain() # need at least 2GB
brain_hash = brain_plasma_hash.Brain()
...
method = brah
code = "brain_hash['x'] = pd.DataFrame({a:range(10000000) for a in 'abcdefghij'})"
for n in (1,2,3):
    start = time.time()
    brain['x'] = pd.DataFrame({a:range(10000000) for a in 'abcdefghij'})
    show(start,task,method,code,n)
...
method = brah
code = "brain_hash['x'] = [medium_json for x in range(1000)]"
for n in (1,2,3):
    start = time.time()
    brain['x'] = [medium_json for x in range(1000)]
    show(start,task,method,code,n)
